...
( function(factory) {"use strict";
		if ( typeof define === 'function' && define.amd) {
			...
			factory(jQuery, Flotr);
		}
	}(function(jQuery, Flotr) {"use strict";

		jQuery.widget("mn.sDashboard", {
			version : "2.5",
			options : {
				dashboardData : []
			},
			...
			_createView : function() {
				...
				jQuery("body").append("<div class='sDashboard-overlay'></div>");

				jQuery(".sDashboard-overlay").height(docHeight);

				jQuery(".sDashboard-overlay").hide();
			...
			_bindEvents : function() {
				...
				this.element.on("click", ".sDashboardWidgetHeader div.sDashboard-icon.sDashboard-circle-plus-icon", function(e) {
					...
					var widgetListItem = jQuery(e.currentTarget).parents("li:first");
					//get the widget Container
					var widget = jQuery(e.currentTarget).parents(".sDashboardWidget:first");
...
debugger;
					...
					jQuery(e.currentTarget).toggleClass("sDashboard-circle-minus-icon");
					//change the tooltip on the maximize/minimize icon buttons
					if (jQuery(e.currentTarget).attr("title") === "Maximize") {
						jQuery(".sDashboard-overlay").show();
						jQuery(e.currentTarget).attr("title", "Minimize");
						self._trigger("widgetMaximized", null, {
							"widgetDefinition" : widgetDefinition
						});
					} else {
						jQuery(".sDashboard-overlay").hide();
						jQuery(e.currentTarget).attr("title", "Maximize");
				...
				this.element.on("click", ".sDashboardWidgetHeader div.sDashboard-icon.sDashboard-refresh-icon", function(e) {
					var widget = jQuery(e.currentTarget).parents("li:first");
				...
				this.element.on("click", ".sDashboardWidgetContent table.sDashboardTableView tbody tr", function(e) {
					var selectedRow = jQuery(e.currentTarget);
			...
			_constructWidget : function(widgetDefinition) {
				...
				var widget = jQuery("<li/>").attr("id", widgetDefinition.widgetId);
				//create a widget container
				var widgetContainer = jQuery("<div/>").addClass("sDashboardWidget");

				//create a widget header
				var widgetHeader = jQuery("<div/>").addClass("sDashboardWidgetHeader sDashboard-clearfix");
				var maximizeButton = jQuery('<div title="Maximize" class="sDashboard-icon sDashboard-circle-plus-icon "></span>');

				var deleteButton = jQuery('<div title="Close" class="sDashboard-icon sDashboard-circle-remove-icon"></div>');
				...
				if (widgetDefinition.hasOwnProperty("enableRefresh") && widgetDefinition.enableRefresh) {
					var refreshButton = jQuery('<div title="Refresh" class="sDashboard-icon sDashboard-refresh-icon "></div>');
					//add refresh button
				...
				var widgetContent = jQuery("<div/>").addClass("sDashboardWidgetContent");

				if (widgetDefinition.widgetType === 'table') {
					var dataTable = jQuery('<table cellpadding="0" cellspacing="0" border="0" class="display sDashboardTableView table table-bordered"></table>');
					widgetContent.append(dataTable);
				} else if (widgetDefinition.widgetType === 'chart') {
					var chart = jQuery('<div/>').addClass("sDashboardChart");
					if (widgetDefinition.getDataBySelection) {
			...
			_renderTable : function(widgetDefinition){
				...
				if(widgetDefinition.widgetType === 'table'){
					...
					var tableDef = {};
					jQuery.extend(tableDef,widgetDefinition.widgetContent);
			...
			destroy : function() {
				...
				jQuery(".sDashboard-overlay").remove();
